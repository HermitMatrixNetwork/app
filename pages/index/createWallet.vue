<template>
	<view class="create-wallet">
		...
		<view class="title">{{ language.text06 }}</view>

		<view class="wallet-name">
			<view class="wallet-name-label">{{ language.text07 }}</view>
			<u-input class="wallet-name-input" :formatter="formatter" :class="{ error: invalidFields.fieldName == 'name' }" v-model="name" :placeholder="language.text76"></u-input>
		</view>

		<view class="wallet-password">
			<view class="wallet-password-label">{{ language.text09 }}</view>
			<view class="wallet-password-item">
				<u-input
					...
					v-model="password"
					:placeholder="language.text10"
					:class="{ error: invalidFields.fieldName == 'password' }"
				></u-input>
				<view
					:class="{ error: invalidFields.fieldName == 'password' }"
					style="height: 100%; display: flex; align-items: center; border-radius: 0 16rpx 16rpx 0; overflow: hidden;"
				>
					<image
			...
			<view class="wallet-password-item" :class="{ error: invalidFields.fieldName == 'checkPassword' }">
				<u-input
					:password="!checkPasswordEye"
					:formatter="formatter"
					:class="{ error: invalidFields.fieldName == 'checkPassword' }"
					class="wallet-password-input"
					v-model="checkPassword"
					:placeholder="language.text11"
				></u-input>
				<view
					:class="{ error: invalidFields.fieldName == 'checkPassword' }"
					style="height: 100%; display: flex; align-items: center; border-radius: 0 16rpx 16rpx 0; overflow: hidden;"
				>
					<image
						:src="checkPasswordEye ? '/static/img/password-eye-open.png' : '/static/img/password-eye-close.png'"
