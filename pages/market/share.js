...
export default {
  show({list,cancelText,sharedata},callback){
    var currentLanguage
    //#ifdef APP-PLUS
    currentLanguage = JSON.parse(plus.storage.getItem('_language')).data.data
    //#endif
    
    //#ifndef APP-PLUS
    currentLanguage = uni.getStorageSync('_language').data
    //#endif
    if(!list){
      list = [{
        'img':'/static/img/mine/copylink.png',
        'text': language[currentLanguage].text12
      },{
        'img':'/static/img/mine/refresh.png',
        'text': language[currentLanguage].text13
      },{
        'img':'/static/img/mine/collection.png',
        'text': language[currentLanguage].text14
      }]
    }
    if(list.length>8){
