    ...
    <custom-header tabUrl="/pages/mine/index" :title="language.text37" class="header">
      <template slot="right">
        <view class="title-right" >
          <text class="save" @click="changeRpc" v-show="currentRpc !== nodeList[selectIndex].link">{{ language.text43 }}</text>
...
<script>
import { DEFAULT_RPC } from '@/config/index.js'
...
export default {
  ...
  methods: {
    ...
    changeRpc() {
      ...
      this.$refs.notify.show('', '切换节点成功', { bgColor: '#275EF1' })
    },
    toEditNode(item) {
      uni.redirectTo({
