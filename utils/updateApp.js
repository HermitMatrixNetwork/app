...
// 发起ajax请求获取服务端版本号
export const getServerNo = function(version, isPrompt, callback) {
	let httpData = {
		version: version
	};
	if (platform == "android") {
	...
	uni.request({
		...
		success: res => {
			...
			if (httpData.type == 1101) {
				...
				console.log('参数', iosCallbackData)
				if (updateFile.version > httpData.version) {
					callback && callback(iosCallbackData);
				} else {
					if(isPrompt) {
						uni.showToast({
							title: '已是最新版本，无需更新！',
...
export default function(isPrompt = true) {
